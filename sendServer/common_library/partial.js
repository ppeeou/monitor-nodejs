...
!function(G) {
  ...
  if (typeof exports != 'undefined' && typeof module != 'undefined' && module.exports) module.exports = _;
  else window._ = _;
  window._p = _;
  ...
  _.pairs = function(obj) {
    var keys = _keys(obj), l = keys.length, res = Array(l);
  ...
  _.omit = function f(d, i) { // (data, key1, key2, key3)은 지원 안됨
    ...
    if (_.isFunction(iter)) {
      ...
      var oKeys = _keys(data), keys = _.wrap_arr(iter);
  ...
  _.box = function (key, value) {
    ...
    function make_selector(el) {
      return _.isString(el) ? el : _.isArray(el) ? map(el, function(val) {
        return (_.isString(val) ? val : (likearr(val) ? val[0] : val).getAttribute('box_selector'));
      }).join('->') : (likearr(el) ? el[0] : el).getAttribute('box_selector');
    }
  };
